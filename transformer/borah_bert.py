...
def load_data():
    ...
    # drop neutral ratings — only clear positive/negative signal
    ratings = ratings[ratings['rating'] != 3].copy()
    ratings['label'] = (ratings['rating'] >= 4).astype(int)

    ml1m_movies = pd.read_csv(
        os.path.join(os.path.dirname(__file__), '..', 'datasets', 'ml-1m', 'movies.dat'),
        sep='::', engine='python',
        names=['movie_id', 'title', 'genres'],
        encoding='latin-1',
    )
    ml1m_movies['title_norm'] = ml1m_movies['title'].apply(_normalize_title)

    movies_db = pd.read_csv(MOVIES_PATH)
    movies_db['title_norm'] = movies_db['Title'].str.lower().str.strip()
    movies_db['text'] = (
        movies_db['Overview'].fillna('') + ' ' + movies_db['Tagline'].fillna('')
    ).str.strip()
    movies_db = movies_db[movies_db['text'] != ''][['title_norm', 'text']]

    merged = (
        ratings
        .merge(ml1m_movies[['movie_id', 'title_norm']], on='movie_id')
        .merge(movies_db, on='title_norm')
    )

    # one label per movie: majority vote across all ratings for that movie
    movie_labels = (
        merged.groupby('title_norm')
        .agg(label=('label', lambda x: int(x.mean() >= 0.5)),
             text=('text', 'first'))
        .reset_index()
    )

    X = movie_labels['text'].tolist()
    y = movie_labels['label'].tolist()
    print(f"  Dataset: {len(X)} movies | pos={sum(y)} neg={len(y)-sum(y)}")
